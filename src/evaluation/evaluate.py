...
class CornerDetectionEvaluator:
    ...
    def compute_localization_error(self, predicted_corners: np.ndarray,
        ...
        for i in range(pred_pixels.shape[0]):
            c_p = np.mean(pred_pixels[i], axis=0)
            a_p = np.arctan2(pred_pixels[i][:, 1] - c_p[1], pred_pixels[i][:, 0] - c_p[0])
            pred_pixels[i] = pred_pixels[i][np.argsort(a_p)]
            
            c_g = np.mean(gt_pixels[i], axis=0)
            a_g = np.arctan2(gt_pixels[i][:, 1] - c_g[1], gt_pixels[i][:, 0] - c_g[0])
            gt_pixels[i] = gt_pixels[i][np.argsort(a_g)]
    ...
    def evaluate_batch(self, predicted_corners: torch.Tensor,
        ...
        mean_error_per_corner = np.mean(errors, axis=0)
...
class InferencePipeline:
    ...
    def enhance_document(self, model: torch.nn.Module,
        ...
        tensor_input = torch.from_numpy(resized).permute(2, 0, 1).unsqueeze(0).to(self.device)
