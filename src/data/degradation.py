...
class DegradationPipeline:
    ...
    def apply_perspective_distortion(self, image: np.ndarray, 
                                      corner_variation: float = 0.1) -> Tuple[np.ndarray, np.ndarray]:
        """
        Apply perspective distortion to simulate non-frontal camera angles.
        
        Args:
            image: Input image (H, W, C)
            corner_variation: Maximum variation of corners as fraction of image size
            
        Returns:
            Tuple of (distorted image, transformation matrix)
        """
        h, w = image.shape[:2]
        ...
        # Perturbed corners
        max_shift = int(max(h, w) * corner_variation)
        dst_pts = np.float32([
            [random.randint(-max_shift, max_shift), random.randint(-max_shift, max_shift)],
            [w - 1 + random.randint(-max_shift, max_shift), random.randint(-max_shift, max_shift)],
            [w - 1 + random.randint(-max_shift, max_shift), h - 1 + random.randint(-max_shift, max_shift)],
            [random.randint(-max_shift, max_shift), h - 1 + random.randint(-max_shift, max_shift)]
        ])
        ...
        degraded = cv2.warpPerspective(image, M, (w, h), borderMode=cv2.BORDER_REPLICATE)
        
        return degraded, M
    
    def apply_lens_distortion(self, image: np.ndarray, 
                               k1: float = -0.3, k2: float = 0.1, 
                               p1: float = 0.0, p2: float = 0.0) -> np.ndarray:
        """
        Apply lens distortion to simulate wide-angle or poor quality lenses.
