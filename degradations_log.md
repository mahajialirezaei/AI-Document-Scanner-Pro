...
* **Dark Binder Margins (Optimized Reintroduction):** Explicitly renders thick, dark, textured borders (simulating black binders or desk edges) directly underneath or adjacent to the synthetic paper edges. While earlier iterations caused geometric confusion, this optimized version actively teaches the network to combat the "Semantic Trap" by rejecting high-contrast background distractors.
* **3D Drop Shadow Distractors:** Renders overlapping objects (distractors) with offset dark shadows beneath them to simulate the 3D depth of objects resting on the paper or desk.
* **Camouflage Polygons:** Injects white or off-white polygons (mimicking white mice, cables, or papers) tangent to the document boundaries to force reliance on physical edge structures rather than simple color contrast.
* **Corner Occlusion by Dark Objects:** Moderated sleeve/clothing occlusions are injected with constrained probabilities and wider spatial margins from true corners to prevent total geometric confidence loss.
